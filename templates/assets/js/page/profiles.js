...
var pic_msg = document.getElementById("pic_msg"); // 獲取 msg 元素
...
document.getElementById("file").addEventListener("change", async function () {
  ...
  if (file) {
    ...
    if (file.size > MAX_FILE_SIZE) {
      // img_data.src = '';
      img_data.style.display = "none"; // 隱藏圖片
    ...
    try {
      ...
      if (fileType && ALLOWED_EXTENSIONS.includes(fileType)) {
        ...
        reader.onload = function (e) {
          ...
          img_data.style.display = "block"; // 顯示圖片
        };
...
function updateAttendTable(selectedStatus) {
  ...
  if (selectedStatus === "全部") {
    document.getElementById("attendance-footer").innerHTML = `
      本學期累計: 上課: ${attendanceCounts["上課"] || 0}  請假: ${
      attendanceCounts["請假"] || 0
    }  曠課: ${attendanceCounts["曠課"] || 0}  停課: ${
      attendanceCounts["停課"] || 0
    }，
      剩餘上課次數: ${
        20 - (attendanceCounts["上課"] || 0) - (attendanceCounts["曠課"] || 0)
      }
    `;
  } else {
    ...
    document.getElementById(
      "attendance-footer"
    ).textContent = `本學期累計: ${selectedStatus}：${count}`;
  }
}
