...
function renderTable(data) {
  ...
  data.forEach(function (item, index) {
    ...
    if (getStatusText(item.cert_status) === "已通過") {
      statusButton = `<div class="btn btn-outline-success">${getStatusText(
        item.cert_status
      )}</div>`;
    } else if (getStatusText(item.cert_status) === "審核中") {
      statusButton = `<div class="btn btn-outline-warning">${getStatusText(
        item.cert_status
      )}</div>`;
    } else if (getStatusText(item.cert_status) === "未通過") {
      // 给未通过按钮添加 data-cert-id 属性用于区分
      statusButton = `<div class="btn btn-outline-danger cert_btn" data-cert-id="${
        item.certrecord_id
      }">${getStatusText(item.cert_status)}</div>`;
    }
...
function bindDangerButtonClick(data) {
  ...
  document
    .getElementById("cert-table-body")
    .addEventListener("click", function (event) {
      if (event.target.classList.contains("btn-outline-danger")) {
        ...
        $("#cert_date_edit").val(
          moment(certData.cert_date).format("YYYY-MM-DD")
        );
        $("#file_edit").val();
...
function updateRecommendedCertDropdown(element, filter) {
  ...
  let dropdownItems = document.querySelectorAll(
    "#dropdownMenuButton2 + .dropdown-menu .dropdown-item"
  );
  dropdownItems.forEach((item) => item.classList.remove("active"));
...
document
  .getElementById("uploadButton_edit")
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = function (event) {
      ...
      for (const [type, magic] of Object.entries(magicNumbers)) {
        if (
          bytes
            .slice(0, magic.length)
            .every((val, index) => val === magic[index])
        ) {
          detectedType = type;
          break;
        }
...
document.getElementById("file").addEventListener("change", async function () {
  ...
  if (file) {
    ...
    if (file.size > MAX_FILE_SIZE) {
      ...
      pic_msg.textContent = `上傳圖片過大，圖片大小最大為 ${
  ...
  .addEventListener("change", async function () {
    ...
    if (file) {
      ...
      if (file.size > MAX_FILE_SIZE) {
        ...
        pic_msg_edit.textContent = `上傳圖片過大，圖片大小最大為 ${
          MAX_FILE_SIZE / 1024 / 1024
        } MB。`;
